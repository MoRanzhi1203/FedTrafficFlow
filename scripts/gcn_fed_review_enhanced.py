"""
GCN + BiLSTM + Multi-Head Attention 联邦交通流预测模型（一审增强版）
...
模型架构（保持 GCN + BiLSTM + Multi-Head Attention 基础框架）：
...
def parse_args():
    parser = argparse.ArgumentParser(description="GCN + BiLSTM + Attention 联邦交通流预测（增强版）")
    ...
    parser.add_argument("--output-dir", type=str, default="results/gcn_fed_review_enhanced")
...
class GCN_LSTM_Attention(nn.Module):
    """Full: GCN + BiLSTM + Multi-Head Attention"""
    def __init__(self, K, T, hidden_dim=DEFAULT_HIDDEN_DIM, num_heads=DEFAULT_NUM_HEADS):
        ...
        self.gcn_enc = GCNEncoder(K, T, hidden_dim)
...
class GCN_LSTM_NoAttention(nn.Module):
    """消融: GCN + LSTM (无 Attention)"""
    def __init__(self, K, T, hidden_dim=DEFAULT_HIDDEN_DIM):
        super().__init__()
        self.gcn_enc = GCNEncoder(K, T, hidden_dim)
        self.lstm_enc = BiLSTMEncoder(K, hidden_dim)
...
MODEL_BUILDERS = {
    "full": lambda K, T: GCN_LSTM_Attention(K, T),
    "no_attention": lambda K, T: GCN_LSTM_NoAttention(K, T),
    "lstm_only": lambda K, T: LSTM_Only(K, T),
    "spatial_only": lambda K, T: GCN_Only(K, T),
