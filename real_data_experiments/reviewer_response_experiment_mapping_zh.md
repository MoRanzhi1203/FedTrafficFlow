...
| **消融实验** | 实验 2/4/6 | Exp2: 代码存在（`sia_core.py`），历史成功运行但目录已删除，需恢复或重跑。Exp4: 代码已补全（`rfc_ablation_core.py` + `rfc_ablation_config.py`），已通过 all variants 1k r1e1 smoke；formal 尚未运行。Exp6: scaler 已修复，仅 full variant；待补全 without_attention/without_cnn/without_lstm | P1: 补跑 exp2 formal。P2: Exp4 先跑 r5 diagnostic，若收敛正常再进入 r20 formal。P0: Exp6 scaler 已修复 ✅，下一步补全 4 个 variant |
