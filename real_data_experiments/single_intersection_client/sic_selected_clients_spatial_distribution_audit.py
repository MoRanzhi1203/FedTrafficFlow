...
def render_report(
    ...
    lines = [
        ...
        f"- 与 `cluster-level client` 的区别：{'当前仅覆盖 grid-cell-level；cluster-level 需把多个 grid cells 聚为一个 client，尚未完成。' if cluster_level_mentioned else '当前仅覆盖 grid-cell-level；cluster-level client 本阶段未展开。'}",
        "",
        "## 3. client 基础信息",
        "",
        pipe_table(
