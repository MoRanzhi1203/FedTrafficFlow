﻿﻿# manuscript_sections_zh 目录说明

- current/: 当前用于论文正文的正式草稿版本。
- history/: 历史版本、参考版本、错误结构版本、审查记录和备份文件。
- formula_*.md: 公式说明和符号说明材料。
- real_data_missingness_experiment_design_zh.md: 真实数据缺失值注入实验设计说明。
- real_data_missingness_experiment_results_zh.md: 真实数据缺失值注入实验结果说明。
- real_data_preprocessing_audit_zh.md: 真实数据预处理扫描审计报告。
- real_data_preprocessing_experiment_results_zh.md: 真实数据预处理与数据质量分析正式草稿。
- real_data_preprocessing_scan_list.txt: 更新后的真实数据扫描文件列表（2026-06-10 更新）。
- simulation_experiment_multi_seed_review_zh.md: 多随机种子实验审查材料。

### 更新时间：2026-06-10
审计报告与正式草稿已基于最新审计脚本 (`analysis_scripts/audit_real_data_preprocessing.py`) 运行结果更新，旧版本移入 `history/`。
