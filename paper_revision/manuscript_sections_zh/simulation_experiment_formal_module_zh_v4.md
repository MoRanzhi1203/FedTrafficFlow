...
为更直观地说明基础仿真数据集与增强仿真数据集的构造差异，图 1 给出了仿真交通流数据集构造与 Non-IID 设置示意图。基础仿真数据集用于在较受控的交通流场景下验证 FedAvg 的基本联邦训练效果；增强仿真数据集进一步引入样本量不平衡、分布族差异、噪声水平差异、高峰模式差异和事件扰动，用于模拟更复杂的跨区域 Non-IID 联邦交通预测环境。

![图 1 仿真交通流数据集构造与 Non-IID 设置示意图](../../../results/simulation_experiments/dataset_overview/simulation_dataset_construction_overview.png)

注：图 1 展示了本文基础仿真数据集与增强仿真数据集的构造差异。基础仿真数据集包含 5 个客户端、8 个交通节点、每客户端 200 个样本、输入窗口 24 和预测步长 1；增强仿真数据集进一步引入客户端样本量不平衡、分布族差异、噪声水平差异、高峰模式差异和事件扰动，用于构造更复杂的 Non-IID 联邦交通预测场景。
...
![图 2 CNN-FedAvg 五随机种子主指标结果](../../../results/simulation_experiments/cnn_fed_base/multi_seed_mean_std_metrics.png)

注：图 2 给出了 CNN-FedAvg 在五个随机种子下的主指标均值与标准差，用于展示不同随机初始化条件下模型预测结果的波动范围。
...
![图 3 GCN-FedAvg 五随机种子主指标结果](../../../results/simulation_experiments/gcn_fed_base/multi_seed_mean_std_metrics.png)

注：图 3 给出了 GCN-FedAvg 在五个随机种子下的主指标均值与标准差，用于展示图结构建模条件下模型误差及其波动范围。

![图 4 CNN-FedAvg 收敛曲线](../../../results/simulation_experiments/cnn_fed_base/convergence_curve.png)

注：图 4 展示了 CNN-FedAvg 在通信轮次推进过程中的验证误差变化趋势。

![图 5 GCN-FedAvg 收敛曲线](../../../results/simulation_experiments/gcn_fed_base/convergence_curve.png)

注：图 5 展示了 GCN-FedAvg 在通信轮次推进过程中的验证误差变化趋势。
...
注：图 6 展示了不同 Non-IID 程度下 FedAvg 的误差变化趋势。
...
注：图 7 展示了不同客户端数量条件下 FedAvg 的误差变化趋势。
...
注：图 8 展示了不同输入特征组合下 FedAvg 的误差变化趋势。
...
![图 9 FedAvg 在联邦扰动场景下的 RMSE 稳定性](../../../results/simulation_experiments/fed_robustness_experiments/fedavg_robustness_rmse_mean_std.png)

注：图 9 展示了 FedAvg 在客户端掉线、通信延迟和梯度扰动三类场景下的 RMSE 均值与标准差。该图仅展示标准 FedAvg 的多随机种子结果，用于说明不同扰动场景下的误差变化趋势。
...
![图 10 基础 GCN 邻接矩阵示意](../../../results/simulation_experiments/gcn_fed_base/base_graph_adjacency_matrix.png)

注：图 10 展示了基础 GCN 实验所采用的邻接矩阵结构。

![图 11 增强 GCN 固定邻接矩阵示意](../../../results/simulation_experiments/gcn_fed_enhanced_experiments/enhanced_gcn_fixed_adjacency.png)

注：图 11 展示了增强 GCN 实验所采用的固定图邻接矩阵结构。
...
注：图 12 基于单一随机种子结果，仅用于展示固定图与动态图设置下的趋势性差异，不作为强统计结论。

## 图表、表格与伪代码安排

本文正文图表围绕主实验结论展开，误差分布、随机种子配对关系和单类扰动曲线作为补充材料展示。正文首先给出仿真数据集构造与 Non-IID 设置示意图，用于说明基础仿真数据集与增强仿真数据集的差异；随后展示基础主结果图、收敛曲线、增强异质性趋势图、鲁棒性 RMSE 汇总图以及 GCN 图结构示意图。附录保留 RMSE 箱线图、随机种子配对图和多指标扰动补充图。正文图表均围绕标准 FedAvg 聚合展开，不将探索性加权策略图件纳入主文结论。
