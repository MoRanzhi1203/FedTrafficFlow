...
基础 CNN 与基础 GCN 实验采用一致的数据生成框架。基础场景包含 5 个客户端、8 个交通节点、长度为 24 的输入窗口和 1 步预测步长，训练集、验证集和测试集划分比例为 70%/10%/20%。为避免过度理想化的同构假设，本文在基础仿真数据集中引入轻度样本量不平衡和受控客户端差异：五个客户端的样本量分别为 180、190、200、210 和 220，同时在流量水平、高峰强度、相位偏移和观测噪声方面设置轻中度差异。该设计用于构造受控但不完全相同的弱异质性联邦交通预测场景，并与增强仿真中的强 Non-IID 设置形成区分。
...
为更直观地说明仿真数据的构造逻辑，图 1 给出了基础仿真数据集构造示意图。基础仿真数据集用于在轻度样本量不平衡、空间结构一致和输入输出设置一致的条件下，验证 FedAvg 的基本协同建模能力；与此同时，各客户端平均交通流轨迹和目标流量分布保留可观察但受控的轻中度差异，以构成受控弱异质性的联邦场景。两张数据集说明图采用一致的子图顺序，分别从客户端样本量、平均交通流轨迹、目标流量分布和结构/异质性驱动因素四个角度展示基础场景与增强场景的差异。

![图 1 基础仿真数据集构造示意图](../../../results/simulation_experiments/dataset_overview/base_simulation_dataset_overview.png)

注：图 1 展示了基础仿真交通流数据集的构造特征。该数据集在节点规模和输入输出设置上保持一致，同时引入轻度样本量不平衡以及可观察但受控的流量水平差异、高峰强度差异、相位偏移和噪声差异，用于构造弱异质性的受控联邦交通预测场景。
...
注：图 2 展示了增强仿真数据集中的 Non-IID 构造方式。与基础数据集相比，增强数据集进一步引入更明显的样本量不平衡、分布族差异、噪声水平差异、高峰模式差异和事件扰动，用于模拟更复杂的跨区域联邦交通预测环境。
...
五随机种子结果显示，在 CNN 基础实验中，FedAvg 的 RMSE、MAE 和 MAPE 分别为 `0.0605 ± 0.0052`、`0.0493 ± 0.0052` 和 `4.9328 ± 0.5192`；Independent 的对应指标为 `0.0801 ± 0.0183`、`0.0664 ± 0.0149` 和 `6.6412 ± 1.4906`。该结果表明，在轻度样本量不平衡、空间结构一致且仅保留弱异质性的基础场景中，FedAvg 相较独立训练仍能稳定降低总体误差，说明跨客户端协同建模能够在受控弱异质性条件下形成有效收益。
...
| 每客户端样本 | 180 / 190 / 200 / 210 / 220 | 180 / 190 / 200 / 210 / 220 | 600/500/700/550/450 | 与增强数据一致 | 与增强数据一致 |
...
|---|---:|---:|---:|---:|---|
| FedAvg | 0.0605 ± 0.0052 | 0.0493 ± 0.0052 | 4.9328 ± 0.5192 | 0.8854 ± 0.0230 | 主要误差指标更低 |
| Independent | 0.0801 ± 0.0183 | 0.0664 ± 0.0149 | 6.6412 ± 1.4906 | 0.7871 ± 0.0892 | 整体误差更高 |
...
五随机种子结果显示，在 GCN 基础实验中，FedAvg 的 RMSE、MAE 和 MAPE 分别为 `0.0533 ± 0.0043`、`0.0438 ± 0.0038` 和 `4.3783 ± 0.3758`；Independent 的对应指标为 `0.0901 ± 0.0103`、`0.0746 ± 0.0092` 和 `7.4552 ± 0.9197`。该结果说明，在显式图结构输入下，FedAvg 相较独立训练同样表现出更低的整体误差，而且这一优势在轻度样本量不平衡的弱异质性基础场景中更加稳定。

**表 3 GCN 基础实验五随机种子结果**

| 方法 | RMSE | MAE | MAPE | R² | 结果说明 |
|---|---:|---:|---:|---:|---|
| FedAvg | 0.0533 ± 0.0043 | 0.0438 ± 0.0038 | 4.3783 ± 0.3758 | 0.9087 ± 0.0185 | 整体误差更低且更稳定 |
| Independent | 0.0901 ± 0.0103 | 0.0746 ± 0.0092 | 7.4552 ± 0.9197 | 0.7494 ± 0.0475 | 整体误差更高 |
...
| 设置 | RMSE | MAE | MAPE | R² |
|---|---:|---:|---:|---:|
| FedAvg-CNN | 0.0605 ± 0.0052 | 0.0493 ± 0.0052 | 4.9328 ± 0.5192 | 0.8854 ± 0.0230 |
| FedAvg-GCN | 0.0533 ± 0.0043 | 0.0438 ± 0.0038 | 4.3783 ± 0.3758 | 0.9087 ± 0.0185 |
...
基础实验的收敛结果需要区分 CNN 与 GCN 两条路径。CNN 基础实验的主训练轮次为 10，但收敛轨迹记录了 15 轮；GCN 基础实验的训练与收敛记录均为 10 轮。CNN 的平均验证 RMSE 从第 1 轮的 `0.2917` 快速下降到第 5 轮附近的 `0.0642`，并在第 10 至 15 轮稳定在约 `0.0627` 至 `0.0759` 区间；GCN 的平均验证 RMSE 则从第 1 轮的 `0.0606` 下降到第 10 轮的 `0.0536`。

这说明标准 FedAvg 在合成交通流场景中能够在有限通信轮次内形成稳定的全局模型，而且这种收敛稳定性在 CNN 与 GCN 两类空间模块下均可观察到。因此，收敛收益主要来自统一的联邦训练协议，而不是某一特定局部模型的偶然表现。

多随机种子收敛统计结果显示，基础 CNN 的最终轮验证集 RMSE 为 `0.0627 ± 0.0074`，基础 GCN 的最终轮验证集 RMSE 为 `0.0536 ± 0.0036`，进一步表明基础 FedAvg 在不同随机初始化条件下仍能保持较稳定的收敛终点。增强实验的收敛统计结果还表明，CNN 增强实验中 FedAvg 的最终轮验证集 RMSE 为 `7.3178 ± 0.6780`，对应增强设置的结果为 `7.3146 ± 0.6804`；GCN 增强实验中 FedAvg 的最终轮验证集 RMSE 为 `7.5583 ± 0.5910`，对应增强设置的结果为 `7.4103 ± 0.6005`。这些结果说明，标准 FedAvg 在基础场景和增强场景下均能够形成可重复的收敛趋势。
