...
基础 CNN 与基础 GCN 实验采用一致的数据生成框架。基础场景包含 5 个客户端、8 个交通节点、长度为 24 的输入窗口和 1 步预测步长；每个客户端包含 200 个样本，训练集、验证集和测试集划分比例为 70%/10%/20%。需要说明的是，基础仿真数据集并非要求各客户端样本完全一致，而是在样本量、节点规模和输入输出设置保持一致的前提下，引入可观察但受控的轻中度客户端差异，包括流量水平差异、高峰强度差异、相位偏移和观测噪声差异，从而形成均衡弱异质性的联邦交通预测场景。该设计既避免了完全同质数据过度理想化的问题，也与增强仿真中的强 Non-IID 设置形成区分。
...
为更直观地说明仿真数据的构造逻辑，图 1 给出了基础仿真数据集构造示意图。基础仿真数据集用于在样本量均衡、空间结构一致和输入输出设置一致的条件下，验证 FedAvg 的基本协同建模能力；与此同时，各客户端平均交通流轨迹和目标流量分布保留可观察但受控的轻中度差异，以构成均衡弱异质性的联邦场景。

![图 1 基础仿真数据集构造示意图](../../../results/simulation_experiments/dataset_overview/base_simulation_dataset_overview.png)

注：图 1 展示了基础仿真交通流数据集的构造特征。该数据集在客户端样本量、节点规模和输入输出设置上保持均衡，同时各客户端平均交通流曲线和目标流量分布存在可观察但受控的轻中度差异，用于构造均衡弱异质性的联邦交通预测场景。
...
注：图 2 展示了增强仿真数据集中的 Non-IID 构造方式。与基础数据集相比，增强数据集进一步引入样本量不平衡、分布族差异、噪声水平差异、高峰模式差异和事件扰动，用于模拟更复杂的跨区域联邦交通预测环境。
...
五随机种子结果显示，在 CNN 基础实验中，FedAvg 的 RMSE、MAE 和 MAPE 分别为 `0.0709 ± 0.0054`、`0.0568 ± 0.0049` 和 `5.6802 ± 0.4936`；Independent 的对应指标为 `0.0897 ± 0.0029`、`0.0742 ± 0.0026` 和 `7.4201 ± 0.2622`。该结果表明，在样本量均衡、空间结构一致且仅保留弱异质性的基础场景中，FedAvg 相较独立训练仍能稳定降低总体误差，说明跨客户端协同建模能够在受控弱异质性条件下形成有效收益。
...
|---|---:|---:|---:|---:|---|
| FedAvg | 0.0709 ± 0.0054 | 0.0568 ± 0.0049 | 5.6802 ± 0.4936 | 0.8446 ± 0.0242 | 主要误差指标更低 |
| Independent | 0.0897 ± 0.0029 | 0.0742 ± 0.0026 | 7.4201 ± 0.2622 | 0.7576 ± 0.0152 | 整体误差更高 |
...
五随机种子结果显示，在 GCN 基础实验中，FedAvg 的 RMSE、MAE 和 MAPE 分别为 `0.0589 ± 0.0030`、`0.0474 ± 0.0027` 和 `4.7352 ± 0.2699`；Independent 的对应指标为 `0.0912 ± 0.0079`、`0.0753 ± 0.0069` 和 `7.5264 ± 0.6941`。该结果说明，在显式图结构输入下，FedAvg 相较独立训练同样表现出更低的整体误差，而且这一优势在弱异质性的基础场景中更加稳定。

**表 3 GCN 基础实验五随机种子结果**

| 方法 | RMSE | MAE | MAPE | R² | 结果说明 |
|---|---:|---:|---:|---:|---|
| FedAvg | 0.0589 ± 0.0030 | 0.0474 ± 0.0027 | 4.7352 ± 0.2699 | 0.8902 ± 0.0193 | 整体误差更低且更稳定 |
| Independent | 0.0912 ± 0.0079 | 0.0753 ± 0.0069 | 7.5264 ± 0.6941 | 0.7474 ± 0.0500 | 整体误差更高 |
...
| 设置 | RMSE | MAE | MAPE | R² |
|---|---:|---:|---:|---:|
| FedAvg-CNN | 0.0709 ± 0.0054 | 0.0568 ± 0.0049 | 5.6802 ± 0.4936 | 0.8446 ± 0.0242 |
| FedAvg-GCN | 0.0589 ± 0.0030 | 0.0474 ± 0.0027 | 4.7352 ± 0.2699 | 0.8902 ± 0.0193 |

由表 4 可知，GCN-FedAvg 在五个随机种子下的 RMSE、MAE 和 MAPE 均低于 CNN-FedAvg，且波动更小，说明图结构建模在当前基础实验设置下具有更稳定的统计表现。与此同时，两条路径的 `R²` 均为正值，说明在均衡弱异质性的受控场景中，标准 FedAvg 已能够较稳定地解释目标序列方差；但综合结论仍应结合 RMSE、MAE、MAPE 以及收敛趋势共同判断。
...
基础实验的收敛结果需要区分 CNN 与 GCN 两条路径。CNN 基础实验的主训练轮次为 10，但收敛轨迹记录了 15 轮；GCN 基础实验的训练与收敛记录均为 10 轮。CNN 的平均验证 RMSE 从第 1 轮的 `0.2921` 快速下降到第 5 轮附近的 `0.0751`，并在第 10 至 15 轮稳定在约 `0.0741` 至 `0.0759` 区间；GCN 的平均验证 RMSE 则从第 1 轮的 `0.0697` 下降到第 10 轮的 `0.0589`。

这说明标准 FedAvg 在合成交通流场景中能够在有限通信轮次内形成稳定的全局模型，而且这种收敛稳定性在 CNN 与 GCN 两类空间模块下均可观察到。因此，收敛收益主要来自统一的联邦训练协议，而不是某一特定局部模型的偶然表现。

多随机种子收敛统计结果显示，基础 CNN 的最终轮验证集 RMSE 为 `0.0746 ± 0.0079`，基础 GCN 的最终轮验证集 RMSE 为 `0.0589 ± 0.0032`，进一步表明基础 FedAvg 在不同随机初始化条件下仍能保持较稳定的收敛终点。增强实验的收敛统计结果还表明，CNN 增强实验中 FedAvg 的最终轮验证集 RMSE 为 `7.3178 ± 0.6780`，对应增强设置的结果为 `7.3146 ± 0.6804`；GCN 增强实验中 FedAvg 的最终轮验证集 RMSE 为 `7.5583 ± 0.5910`，对应增强设置的结果为 `7.4103 ± 0.6005`。这些结果说明，标准 FedAvg 在基础场景和增强场景下均能够形成可重复的收敛趋势。
