...
为更直观地说明仿真数据的构造逻辑，图 1 给出了基础仿真数据集构造示意图。基础仿真数据集用于受控、均衡条件下验证 FedAvg 的基本有效性，其客户端样本量保持一致，平均交通流时间序列模式相对接近，并与基础 8 节点路网结构共同构成标准化的联邦实验环境。

![图 1 基础仿真数据集构造示意图](../../../results/simulation_experiments/dataset_overview/base_simulation_dataset_overview.png)

注：图 1 展示了基础仿真数据集的构造方式，包括 5 个客户端均衡样本量、各客户端平均交通流时间序列、24 步输入到 1 步预测的窗口设定，以及基础 8 节点路网邻接结构。该图用于说明基础仿真数据集在受控、相对均衡条件下的实验设置。
...
注：图 2 展示了增强仿真数据集的多源异构性，包括客户端样本量不平衡、目标流量分布差异、平均交通流时间序列中的峰型差异，以及噪声水平、峰值强度和事件扰动概率的联合差异。该图用于说明增强仿真数据集如何构造更复杂的 Non-IID 联邦交通预测场景。
...
本文正文图表围绕主实验结论展开，误差分布、随机种子配对关系和单类扰动曲线作为补充材料展示。正文首先给出基础仿真数据集构造示意图和增强仿真数据集 Non-IID 构造示意图，用于说明受控均衡场景与复杂异质场景之间的差异；随后展示基础主结果图、收敛曲线、增强异质性趋势图、鲁棒性 RMSE 汇总图以及 GCN 图结构示意图。附录保留 RMSE 箱线图、随机种子配对图和多指标扰动补充图。正文图表均围绕标准 FedAvg 聚合展开，不将探索性加权策略图件纳入主文结论。
