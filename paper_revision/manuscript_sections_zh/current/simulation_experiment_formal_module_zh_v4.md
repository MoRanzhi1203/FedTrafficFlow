...
五随机种子结果显示，在 CNN 基础实验中，FedAvg 的 RMSE、MAE 和 MAPE 分别为 `0.0144 ± 0.0025`、`0.0118 ± 0.0021` 和 `1.1790 ± 0.2104`；Independent 的对应指标为 `0.0174 ± 0.0016`、`0.0147 ± 0.0016` 和 `1.4746 ± 0.1585`。由此可见，在当前基础 CNN 设置下，FedAvg 在主要误差指标上低于独立训练，说明跨客户端参数聚合能够带来一定协同收益。
...
|---|---:|---:|---:|---:|---|
| FedAvg | 0.0144 ± 0.0025 | 0.0118 ± 0.0021 | 1.1790 ± 0.2104 | -0.3849 ± 0.4127 | 主要误差指标更低 |
| Independent | 0.0174 ± 0.0016 | 0.0147 ± 0.0016 | 1.4746 ± 0.1585 | -1.1746 ± 0.3980 | 跨随机种子波动较大 |

![图 1 CNN-FedAvg 五随机种子主指标结果](../../../results/simulation_experiments/cnn_fed_base/multi_seed_mean_std_metrics.png)

注：图 1 给出了 CNN-FedAvg 在五个随机种子下的主指标均值与标准差，用于展示不同随机初始化条件下模型预测结果的波动范围。

### GCN-FedAvg 与 Independent 对比

五随机种子结果显示，在 GCN 基础实验中，FedAvg 的 RMSE、MAE 和 MAPE 分别为 `0.0132 ± 0.0006`、`0.0105 ± 0.0005` 和 `1.0467 ± 0.0496`；Independent 的对应指标为 `0.0150 ± 0.0007`、`0.0124 ± 0.0007` 和 `1.2388 ± 0.0687`。该结果说明，在显式图结构输入下，FedAvg 相较于独立训练同样表现出更低的整体误差。

**表 3 GCN 基础实验五随机种子结果**

| 方法 | RMSE | MAE | MAPE | R² | 结果说明 |
|---|---:|---:|---:|---:|---|
| FedAvg | 0.0132 ± 0.0006 | 0.0105 ± 0.0005 | 1.0467 ± 0.0496 | -0.1114 ± 0.1220 | 整体误差更低且更稳定 |
| Independent | 0.0150 ± 0.0007 | 0.0124 ± 0.0007 | 1.2388 ± 0.0687 | -0.4900 ± 0.1660 | 整体误差更高 |

![图 2 GCN-FedAvg 五随机种子主指标结果](../../../results/simulation_experiments/gcn_fed_base/multi_seed_mean_std_metrics.png)

注：图 2 给出了 GCN-FedAvg 在五个随机种子下的主指标均值与标准差，用于展示图结构建模条件下模型误差及其波动范围。

![图 3 CNN-FedAvg 收敛曲线](../../../results/simulation_experiments/cnn_fed_base/convergence_curve.png)

注：图 3 展示了 CNN-FedAvg 在通信轮次推进过程中的验证误差变化趋势。

![图 4 GCN-FedAvg 收敛曲线](../../../results/simulation_experiments/gcn_fed_base/convergence_curve.png)

注：图 4 展示了 GCN-FedAvg 在通信轮次推进过程中的验证误差变化趋势。

### CNN 与 GCN 路径对比

五随机种子统计结果显示，GCN-FedAvg 在 RMSE、MAE 和 MAPE 上均低于 CNN-FedAvg，且标准差更小，说明图结构建模在当前实验设置下具有更稳定的统计表现。与此同时，CNN 路径在规则局部空间结构中仍具有较明确的归纳偏置，因此两类结构可以作为标准 FedAvg 下的互补空间建模路径。
...
注：图 5 展示了不同 Non-IID 程度下 FedAvg 的误差变化趋势。
...
注：图 6 展示了不同客户端数量条件下 FedAvg 的误差变化趋势。
...
注：图 7 展示了不同输入特征组合下 FedAvg 的误差变化趋势。
...
![图 8 FedAvg 在联邦扰动场景下的 RMSE 稳定性](../../../results/simulation_experiments/fed_robustness_experiments/fedavg_robustness_rmse_mean_std.png)

注：图 8 展示了 FedAvg 在客户端掉线、通信延迟和梯度扰动三类场景下的 RMSE 均值与标准差。该图仅展示标准 FedAvg 的多随机种子结果，用于说明不同扰动场景下的误差变化趋势。

## GCN 图结构与动态图分析

### 9.1 增强 GCN 默认场景的五随机种子结果
...
注：图 11 基于单一随机种子结果，仅用于展示固定图与动态图设置下的趋势性差异，不作为强统计结论。

## 图表、表格与伪代码安排

本文正文图表围绕主实验结论展开，误差分布、随机种子配对关系和单类扰动曲线作为补充材料展示。正文保留基础主结果图、收敛曲线、增强异质性趋势图、鲁棒性 RMSE 汇总图以及 GCN 图结构示意图；附录保留 RMSE 箱线图、随机种子配对图和多指标扰动补充图。正文图表均围绕标准 FedAvg 聚合展开，不将探索性加权策略图件纳入主文结论。
