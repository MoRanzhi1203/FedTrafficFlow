"""生成全局随机缺失场景所需的掩码、缺失数据与审计产物。

核心功能：
- 基于完整节点车流量分块数据按给定缺失率构造 global MCAR 掩码；
- 输出缺失数据、掩码文件、manifest、运行配置和审计结果；
- 为后续补全过程与评估流程提供可复现、可追踪的输入数据。

项目作用：
- 作为全局随机缺失实验的 setting 阶段脚本；
- 保证缺失生成逻辑、缺失率统计和运行配置统一落盘。

关键依赖：`numpy`、`pandas`、`pyarrow`。
主要输入：完整 parquet 分块、缺失率、随机种子和列配置。
主要输出：掩码文件、缺失数据、统计清单和审计报告。
"""
