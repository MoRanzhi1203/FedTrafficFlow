"""分析结构化缺失场景的分布特征并输出审计结果。

核心功能：
- 汇总生成缺失掩码后的节点、时间段、长度组分布；
- 输出 CSV、JSON、Markdown 与图像结果，便于检查缺失设计是否符合预期；
- 为后续补全评估、实验整理和可视化提供统一统计输入。

项目作用：
- 作为结构化缺失设置阶段后的质量审计脚本；
- 帮助定位缺失率偏差、长度分组失衡和时间分布异常。

关键依赖：`pandas`、`numpy`、`matplotlib`。
主要输入：结构化缺失实验目录及其 manifest / audit 文件。
主要输出：分布统计表、审计报告和诊断图像。
"""
