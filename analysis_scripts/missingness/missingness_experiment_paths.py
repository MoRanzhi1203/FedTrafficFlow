"""统一解析缺失实验目录、注册表与历史路径别名。

核心功能：
- 提供场景目录、缺失设置目录、补全目录和 comparison 目录的统一访问接口；
- 读取实验注册表与路径别名文件，解析迁移后的真实结果位置；
- 为分析、修复、整理和可视化脚本提供稳定的路径层。

项目作用：
- 降低目录迁移后脚本路径耦合带来的维护成本；
- 确保旧路径引用可以被正确映射到当前实验布局。

关键依赖：`json`、`pathlib`。
主要输入：项目根目录、实验根目录、场景标识和历史路径。
主要输出：规范化的 `Path` 对象、注册表记录和路径映射结果。
"""
