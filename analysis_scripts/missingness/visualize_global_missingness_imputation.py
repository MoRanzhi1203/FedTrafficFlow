"""可视化全局随机缺失补全实验的结果与误差分布。

核心功能：
- 读取全局缺失实验的汇总表、明细表和分组统计结果；
- 生成方法对比图、误差分布图和辅助展示结果；
- 输出可视化审计文件，支撑结果复查。

项目作用：
- 作为 global missingness 实验的标准展示脚本；
- 为方法效果分析与报告输出提供统一图形入口。

关键依赖：`pandas`、`numpy`、`matplotlib`。
主要输入：全局缺失实验的 summary 与 audit 产物。
主要输出：图像文件、辅助表和可视化审计结果。
"""
