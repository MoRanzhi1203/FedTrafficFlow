"""生成结构化缺失场景的掩码、缺失数据和设计审计结果。

核心功能：
- 按长度组、节点集合和时间块约束采样结构化缺失事件；
- 输出掩码文件、缺失分块、manifest 和设计说明；
- 为后续结构化缺失补全实验提供可复现输入。

项目作用：
- 作为结构化缺失实验的 setting 阶段核心脚本；
- 统一缺失事件生成逻辑、状态记录和审计落盘方式。

关键依赖：`numpy`、`pandas`、`matplotlib`。
主要输入：完整节点车流量分块、缺失率和结构化采样配置。
主要输出：掩码文件、缺失数据、manifest 和审计报告。
"""
