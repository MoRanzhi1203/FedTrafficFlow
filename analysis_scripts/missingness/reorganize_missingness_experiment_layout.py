...
核心功能：
- 将不同场景的缺失设置、补全结果、汇总与审计文件整理到一致目录结构；
- 维护实验注册表与路径别名，保证迁移后结果仍可被检索与引用；
- 在需要时触发结构化场景修复流程，补齐重组所需的中间产物。

项目作用：
- 作为缺失实验整理与归档阶段的总控脚本；
- 统一不同实验分支的落盘方式，降低后续分析与可视化的路径复杂度。

关键依赖：`pandas`、`pathlib`、`shutil`。
主要输入：项目根目录、结果目录和场景元数据。
主要输出：重组后的目录、注册表文件和路径映射文件。
"""
...
try:
    from analysis_scripts.missingness.missingness_experiment_paths import SCENARIO_IDS
    from analysis_scripts.utils.repair_structured_scenario_artifacts import repair_structured_scenarios_under_target_root
except ImportError:
