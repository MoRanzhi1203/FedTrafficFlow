"""执行全局随机缺失场景的因果补全过程与结果汇总。

核心功能：
- 读取 global MCAR 缺失数据、完整分块数据与拓扑信息，执行多种补全方法；
- 记录分块运行状态、明细误差、分组统计和审计信息；
- 输出后续对比分析与可视化所需的汇总文件和图形产物。

项目作用：
- 作为全局随机缺失实验的核心补全脚本；
- 为方法比较、误差分析和最终报告提供标准化结果输出。

关键依赖：`numpy`、`pandas`、`scipy`、`matplotlib`。
主要输入：完整分块、缺失分块、拓扑文件和运行参数。
主要输出：补全后的分块数据、质量汇总表、审计日志和图表。
"""
