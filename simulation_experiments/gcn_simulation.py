# -*- coding: utf-8 -*-
"""Standalone GCN simulation project with isolated outputs."""
...
def run_overview_experiment(output_dir: Path) -> None:
    ...
    for client_id in range(num_clients):
        ...
        fed_clients.append(FederatedClient(client_id, GCNOverviewModel(k=k, t=t), train_loader, test_loader, criterion, lr=1e-3))
...
def run_ablation_experiment(output_dir: Path) -> Dict[str, Dict[str, float]]:
    return run_weighted_ablation(
        ...
        variants=OrderedDict(
            [
                ("GCN-LSTM-Attention", lambda: GCNAblationFull(k=5, t=24, hidden_dim=128, num_heads=4)),
                ("GCN-LSTM", lambda: GCNAblationLSTM(k=5, t=24, hidden_dim=128)),
                ("LSTM-Attention", lambda: LSTMAttentionHetero(k=5, t=24, hidden_dim=128, num_heads=4)),
                ("GCN-Attention", lambda: GCNAblationAttention(k=5, t=24, hidden_dim=128, num_heads=4)),
            ]
        ),
