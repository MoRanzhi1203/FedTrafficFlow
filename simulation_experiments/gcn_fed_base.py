...
def run_data_visualization_base(output_dir: Path) -> None:
    ...
    ax.bar(client_ids, meta["samples_per_client"],
           color=plt.cm.Set2(np.linspace(0, 1, meta["num_clients"])))
    ax.set_xlabel("Client")
    ...
    im = ax.imshow(A_raw, cmap="Blues", aspect="equal", vmin=0, vmax=1)
    ax.set_xlabel("Node ID")
    ax.set_ylabel("Node ID")
    ax.set_title("GCN Base: Fixed Adjacency Matrix")
    plt.colorbar(im, ax=ax, label="Weight")
    # 标注非零值
    for i in range(A_raw.shape[0]):
        for j in range(A_raw.shape[1]):
            if A_raw[i, j] > 0:
                ax.text(j, i, f"{A_raw[i, j]:.2f}", ha="center", va="center", fontsize=7)
    save_figure(fig, output_dir, "base_gcn_adjacency_matrix.png")

    # ── 8. 节点度分布柱状图 ──
    degrees = A_raw.sum(axis=1)
    fig, ax = plt.subplots(figsize=(10, 5))
    bars = ax.bar(range(len(degrees)), degrees, color="#3498db", edgecolor="white")
    ax.set_xlabel("Node ID")
    ax.set_ylabel("Degree")
    ax.set_title("GCN Base: Node Degree Distribution")
...
def run_main_experiment(output_dir: Path) -> None:
    ...
    for idx, metric in enumerate(["mse", "rmse", "mae"]):
        ...
        ax.bar(x - width/2, fed_vals, width, label="FedAvg", color="#2ecc71")
    ...
    fig.suptitle("GCN Base Experiment: FedAvg vs Independent", fontsize=14)
    plt.tight_layout()
...
def run_convergence_experiment(output_dir: Path) -> None:
    ...
    ax.plot(round_data["round"], round_data["avg_train_loss"],
            "o-", label="Avg Train Loss (MSE)", color="#2ecc71", linewidth=2)
    ax.set_xlabel("Communication Round")
    ax.set_ylabel("Train Loss (MSE)", color="#2ecc71")
