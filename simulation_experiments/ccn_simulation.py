...
class WeightedFederatedServer:
    ...
    聚合逻辑同时考虑两类因素：
    1. 数据量权重：样本量更大的客户端对全局模型影响更大；
    2. 损失权重：局部损失更低的客户端被认为更新质量更高。

    最终再引入 0.9 / 0.1 的动量式混合，避免全局参数在单轮内剧烈震荡。
    """
